...
# ---------------------------------------------------------------------------
# PATCH + DEBUG: обход бага customtkinter на Python 3.14 / Tk 9 + логи
# ---------------------------------------------------------------------------
import tkinter as _tk
import customtkinter as ctk
from customtkinter.windows.widgets.core_widget_classes.ctk_base_class import (
    CTkBaseClass as _CTkBase,
)


def _patched_grid(self, **kwargs):
    try:
        opts = self._options({}, kwargs)
    except Exception as e:
        opts = f"<_options raised {e!r}>"
    print(f"[DBG grid] _w={self._w!r}, cls={type(self).__name__}, "
          f"kwargs={kwargs}, options={opts}", flush=True)
    self._last_geometry_manager_call = {
        "function": lambda **kw: _tk.Grid.grid_configure(self, **kw),
        "kwargs": kwargs,
    }
    return _tk.Grid.grid_configure(self, **kwargs)


def _patched_pack(self, **kwargs):
    try:
        opts = self._options({}, kwargs)
    except Exception as e:
        opts = f"<_options raised {e!r}>"
    print(f"[DBG pack] _w={self._w!r}, cls={type(self).__name__}, "
          f"kwargs={kwargs}, options={opts}", flush=True)
    self._last_geometry_manager_call = {
        "function": lambda **kw: _tk.Pack.pack_configure(self, **kw),
        "kwargs": kwargs,
    }
    return _tk.Pack.pack_configure(self, **kwargs)


def _patched_place(self, **kwargs):
    if "width" in kwargs or "height" in kwargs:
        raise ValueError(
            "'width' and 'height' must be passed to the widget constructor, "
            "not to place()"
        )
    print(f"[DBG place] _w={self._w!r}, cls={type(self).__name__}, "
          f"kwargs={kwargs}", flush=True)
    self._last_geometry_manager_call = {
        "function": lambda **kw: _tk.Place.place_configure(self, **kw),
        "kwargs": kwargs,
    }
    return _tk.Place.place_configure(self, **kwargs)


_CTkBase.grid = _patched_grid
_CTkBase.pack = _patched_pack
_CTkBase.place = _patched_place
# ---------------------------------------------------------------------------
...
from core.ctk_theme import (
    ...
    GRAD_START, GRAD_MIDDLE, GRAD_END, apply_ctk_theme,
)
from core.ui import grad_ctkimage, grad_ctkimage_triple, GradientDivider, NavButton
...
class MainApp:
    ...
    def _build_sidebar(self):
        ...
        limg = grad_ctkimage_triple(44, 44, GRAD_START, GRAD_MIDDLE, GRAD_END, radius=13)
        if limg:
        ...
        GradientDivider(sb, GRAD_START, GRAD_MIDDLE, GRAD_END, height=2).grid(
            row=1, column=0, sticky="ew", padx=22, pady=(8, 14))

        nav = ctk.CTkFrame(sb, fg_color="transparent")
        nav.grid(row=2, column=0, sticky="ew", padx=20)
        ...
        for key, i18n_key, fallback, _sub, icon in self.PAGES:
            btn = NavButton(nav, text=f"{icon} {_t(i18n_key, fallback)}",
                            c1=GRAD_START, c2=GRAD_MIDDLE, c3=GRAD_END, width=208, height=44,
                            command=lambda k=key: self._select(k))
            btn.pack(pady=4)
            self._nav[key] = btn

        bottom = ctk.CTkFrame(sb, fg_color="transparent")
        bottom.grid(row=4, column=0, sticky="ew", padx=20, pady=20)
